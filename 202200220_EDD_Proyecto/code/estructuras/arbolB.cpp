...
Avion* BTreeNode::getAvionRegistro(const std::string& k) {
    ...
    return children[i]->getAvionVuelo(k);
}
Avion* BTreeNode::getAvionVuelo(const std::string& k) {
    int i = 0;
    while (i < keys.size() && k > keys[i].vuelo)
        i++;

    if (i < keys.size() && keys[i].vuelo == k)
        return &keys[i];
...
Avion* BTreeNode::getAvionDestino(const std::string& k) {
    int i = 0;
    while (i < keys.size() && k > keys[i].ciudad_destino)
        i++;

    if (i < keys.size() && keys[i].ciudad_destino == k)
        return &keys[i];

    if (isLeaf)
        return nullptr;

    return children[i]->getAvionDestino(k);
}
